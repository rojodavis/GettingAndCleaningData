...
##Load datasets
##Test
x_test <- read.table(paste(curdir, "\\x_test.txt", sep=''), na.strings = "NA")
y_test <- read.table(paste(curdir, "\\y_test.txt", sep=''), na.strings = "NA")
subject_test <- read.table(paste(curdir, "\\subject_test.txt", sep=''), na.strings = "NA")
##Train
x_train <- read.table(paste(curdir, "\\x_train.txt", sep=''), na.strings = "NA")
y_train <- read.table(paste(curdir, "\\y_train.txt", sep=''), na.strings = "NA")
subject_train <- read.table(paste(curdir, "\\subject_train.txt", sep=''), na.strings = "NA")
##Supporting Data
features <- read.table(paste(curdir, "\\features.txt", sep=''), na.strings = "NA")
activity_labels <- read.table(paste(curdir, "\\activity_labels.txt", sep=''), na.strings = "NA")
...
tidy <- combined_tidy_select %>% group_by(Subject, Activity, ActivityD) %>% 
       summarise_if(.predicate = function(x) is.numeric(x),
                     .funs = funs(mean="mean"))
##Write the file
##write.xlsx(as.data.frame(tidy), paste(curdir, "\\Tidy.xlsx", sep=''))
write.table(tidy, file="C:\\tidy.txt", row.name=FALSE)
